"""Expose normalized market-data functions to the LLM."""
...
def tool_get_financials(ticker: str) -> dict:
    ...
    # 免费 Key 的 /quote 不含 PE/市值 → 腾讯备用报价 / stockanalysis 补齐
...
# 公司新闻工具 schema（追加到 TOOL_SCHEMAS，供 Function Calling）
# 估值相对位置工具 schema（追加到 TOOL_SCHEMAS，供 Function Calling）
...
# 对话内出图工具 schema（追加到 TOOL_SCHEMAS，供 Function Calling）
TOOL_SCHEMAS.append({'type': 'function', 'function': {
    'name': 'plot_chart',
